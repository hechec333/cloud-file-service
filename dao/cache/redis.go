...
func Setup(config *config.Config) {
	RedisPool = &redis.Pool{
		...
		Dial: func() (redis.Conn, error) { // 创建链接
			c, err := redis.Dial("tcp", config.RedisConfig.Host+":"+config.RedisConfig.Port)
			if err != nil {
				return nil, err
			}

			_, err = c.Do("AUTH", config.RedisConfig.Password)
			if err != nil {
				return nil, err
			}
			return c, nil
	...
	fmt.Println("Redis init on port ", config.RedisConfig.Host)
}
